...
export default async function Cards({
  ...
  const flashcards = await api.flashcard.getMyFlashCards({
    page: Number(page),
    pageSize: Number(perPage),
  });

  return (
    <div className="container mx-auto flex flex-col items-start gap-5 px-10">
      <h1 className="text-2xl font-medium">My Flashcards</h1>
      ...
      {flashcards.data.length < 1 && (
        <div className="flex h-24 w-full items-center justify-center text-center text-2xl">
          Your flashcard list is empty. You haven&apos;t created any flashcard
          yet.
        </div>
      )}
      {flashcards.data.length > 0 && (
